...
export default class Obstacle extends Vue {
    ...
    createObstacle(){
        ...
        this.obstacle_.left = 650;

        const rnd = random(1, 3);
        const spc = random(70, 180);

        if(!this.top) this.obstacle_.bottom = Math.random() * 60;
        else this.obstacle_.bottom = (Math.random() * 60) + this.obstacle_.gap;

        if(rnd%2 == 1)this.obstacle_.left -=spc;
    }
